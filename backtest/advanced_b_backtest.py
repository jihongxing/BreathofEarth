"""
息壤（Xi-Rang）方案B：动量过滤（200日均线）

基于引擎回测，增加SPY 200日均线动量过滤：
- 如果 SPY < MA200 且 drawdown < -0.08，提前触发保护
- 如果 SPY > MA200 且冷却期满，才解除保护
"""
...
def run_advanced_b_backtest(file_path="data/etf_daily.csv"):
    # 1. 加载数据
    print("加载数据...")
    ...
    # 计算SPY的200日移动平均线
    spy_ma200 = df["SPY"].rolling(window=200).mean()
    ...
    for i in range(len(dates)):
        ...
        if simulated_nav > risk.high_water_mark:
            risk.high_water_mark = simulated_nav
        ...
        # 如果SPY < MA200 且回撤 < -8%，提前触发保护
        if not np.isnan(ma200) and spy_price < ma200 and current_drawdown < -0.08:
            if portfolio.state == STATE_IDLE:
                risk_signal = "PROTECTION"

        # 如果SPY > MA200 且冷却期满，才允许解除保护
        if not np.isnan(ma200) and spy_price < ma200:
            if portfolio.state == STATE_PROTECTION and risk_signal == "IDLE":
                # 保持保护状态，不解除
                risk_signal = "PROTECTION"
    ...
    print()
    print("=" * 60)
    print("  方案B：动量过滤（200日均线）回测报告  2005-2025")
    print("=" * 60)
    ...
    # NAV + 保护区间
    ax1 = axes[0]
    ax1.plot(result.index, result["NAV"], color="#2c3e50", linewidth=1.2, label="Xi-Rang NAV (Plan B)")
    ax1.fill_between(
        result.index,
    ...
    ax1.set_title("方案B：动量过滤（200日均线）", fontsize=14)
    ...
    plt.tight_layout()
    fig.savefig(output_dir / "advanced_b_backtest.png", dpi=150, bbox_inches="tight")
    print(f"\n  图表已保存: {output_dir / 'advanced_b_backtest.png'}")
    plt.close()
